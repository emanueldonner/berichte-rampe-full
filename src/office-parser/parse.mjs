...
import { mkdirhir, makeLog, getLog, initTextTraceOut, textTraceOut } from './lib/lib.js';
...
function cliUsage() {
    ...
    let u = 'Synopsis: parse.mjs [ -H dir1 ] [ -x dir2 ] [ -n dir3 ] [-L level] [-P] [-h] [-T tracefile] [-i] input.docx';
...
function cleanupChapters(dir) {
    ...
    fs.readdirSync(dir).forEach(
        f => f.match(/^chapter_[0-9]{3}$/) && fs.rmSync(`${dir}${path.sep}${f}`, {recursive: true})        
    );
}
...
async function doit(cla) {
    ...
    try {
        ...
        if (! (cla.htmlOut || cla.docxOut || cla.nunjucksOut || cla.textTrace)) {
            log.error({loc: 'cd547f'}, 'One of the options H, x, n or T must be specified');
            return 1;
        }
        ...
        if (cla.htmlOut) {
            ...
            parser.getParseResult().forEach( p => fs.writeSync(output, p.toHtml() + '\n'));
